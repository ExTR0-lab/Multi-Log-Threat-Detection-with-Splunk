...
index=data_exfil specifies the dataset being analyzed.

sourcetype="dns_logs" filters events related to DNS traffic.

stats count by src_ip counts the number of DNS queries generated by each source IP address.

sort -count sorts the results from highest to lowest query volume.
...
len(query) calculates the length of the DNS query.

Queries longer than 30 characters are filtered for further investigation.
